...
<div class="w-full">
    <div class="relative">
        <input type="text" wire:model.live.debounce.200ms="search" placeholder="Search Product..."
    ...
    <div class="bg-white rounded-xl sm:rounded-2xl border border-gray-200 mb-6">
        <div class="p-4 sm:p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                @foreach ($products as $product)
                    <div
                        class="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow duration-300 group">
                        <!-- Product Image -->
                        <div class="relative overflow-hidden bg-gray-100 aspect-square">
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center">
                                    <svg class="w-16 h-16 sm:w-20 sm:h-20 text-gray-300" fill="none"
                            ...
                            @if (isset($product->stock))
                                @if ($product->stock == 0)
                                    <span
                                        class="absolute top-2 right-2 sm:top-3 sm:right-3 bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded-full">
                                        Out of Stock
                                    </span>
                                @elseif($product->stock < 10)
                                    <span
                                        class="absolute top-2 right-2 sm:top-3 sm:right-3 bg-yellow-500 text-white text-xs font-semibold px-2 py-1 rounded-full">
                                        Low Stock
                                    </span>
                                @endif
                            @endif
                        ...
                        <div class="p-3 sm:p-4">
                            ...
                            <!-- Product Name -->
                            <h3
                                class="text-sm sm:text-base font-semibold text-gray-800 mt-2 line-clamp-2 min-h-[2.5rem] sm:min-h-[3rem]">
                                {{ $product->name }}
                            </h3>
            ...
            @if ($products->isEmpty())
                <div class="text-center py-12 sm:py-16">
                    <svg class="w-20 h-20 sm:w-24 sm:h-24 text-gray-300 mx-auto mb-4" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                    </svg>
                    <h3 class="text-lg sm:text-xl font-semibold text-gray-700 mb-2">No Products Yet</h3>
                    <p class="text-sm sm:text-base text-gray-500 mb-6 px-4">Start by adding your first product to the
                        store</p>
        ...
        @if (is_object($products) && method_exists($products, 'hasPages') && $products->hasPages())
            <div
                class="px-4 sm:px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0">
